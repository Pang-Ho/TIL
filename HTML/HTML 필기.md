...
* static
  ...
  나열하는데로 위쪽에서 아래쪽으로 순서대로 배치

* relative

  부모 내부에서 초기 위치에서 상하좌우 위치이동

* absolute

  부모 내부에서 절대 위치 좌표 설정

  부모 내부에서 어느 특정 위치에 있길 바라면서...

* fixed

  브라우저 내부에서 화면을 기준으로 절대 위치 좌표 설정

  브라우저 내부에서 어느 특정 위치에 있길 바라면서...

margin 만으로도 배치를 충분히 할 수 있다!
...
* width:100px / height: 100px 줬더니 상자 안에서 글자가 삐져나왔다.

  그 경우를 overflow라 하여 hidden, scroll, auto 등으로 변경할 수 있다.

### float

* div 특성은 가로크기=브라우저크기, 세로크기=내용크기
* float을 사용하면 브라우저 크기에 상관없이 일정한 위치에 고정시킬 때
